...
class Application {
	...
	function set_skipFrame(val:Bool):Bool {
		if (val) {
			trace("fluid.plugins.app.Application > Deprecated: skipFrame - use fps property and set it to 30 instead");
			fps = 30;
		}
	...
	/**
	 * Starts fluid.application setup using the properties set or default values
	 * @param [rendererType] - Renderer type to use AUTO (default) | CANVAS | WEBGL
	 * @param [stats] - Enable/disable stats for the application.
	 * Note that stats.js is not part of fluid.so don't forget to include it you html page
	...
	public function start(?rendererType:String = "auto", ?parentDom:Element) {
		...
		if (rendererType == AUTO) renderer = Fluid.autoDetectRenderer(width, height, renderingOptions);
